...
def test_main_default_selects_standard_geant4_full_simulation(monkeypatch) -> None:
    ...
    physical_config = load_runtime_config(Path(str(captured["sim_config_path"])))
    pf_config = load_runtime_config(Path(str(captured["pf_config_path"])))
    assert "pure_pf_schema_version" not in physical_config
    assert pf_config["pure_pf_schema_version"] == 1
    assert pf_config["variable_cardinality"] is True
    assert pf_config["pf_max_sources"] == 5
    assert pf_config["structural_cardinality_prior_mean"] == pytest.approx(2.0)
    assert pf_config["estimator_profile"] == "pf_strict"
    assert float(pf_config["pf_strength_prior_min_cps_1m"]) >= 0.0
    assert (
        float(pf_config["pf_strength_prior_max_cps_1m"])
        > float(pf_config["pf_strength_prior_min_cps_1m"])
    )
    assert float(pf_config["structural_rj_move_probability"]) > 0.0
    assert float(pf_config["structural_rj_birth_probability"]) > 0.0
    assert float(pf_config["structural_rj_death_probability"]) > 0.0
