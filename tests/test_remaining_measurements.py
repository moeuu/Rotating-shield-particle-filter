...
def _build_two_mode_estimator() -> RotatingShieldPFEstimator:
    ...
    estimator.measurements.append(
        MeasurementRecord(
            ...
            fe_index=0,
            pb_index=0,
            detector_position_xyz_m=(1.0, 2.0, 0.0),
...
def test_remaining_measurement_reuses_guidance_state_cache() -> None:
    ...
    assert report.components["same_isotope_separation"] == pytest.approx(
        guidance.components["same_isotope_separation"]
    )


def test_remaining_measurement_uses_pf_cardinality_entropy_from_posterior() -> None:
    """A mixed PF source count should stay active from posterior mass alone."""
    isotope = "Eu-154"
    estimator = RotatingShieldPFEstimator(
        isotopes=[isotope],
    ...
    estimator.measurements.append(
        MeasurementRecord(
            ...
            fe_index=0,
            pb_index=0,
            detector_position_xyz_m=(5.0, 5.0, 0.5),
            station_sequence_id=0,
            station_view_index=0,
            runtime_likelihood_route_by_isotope={isotope: "count"},
