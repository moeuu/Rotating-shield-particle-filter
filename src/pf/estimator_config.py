...
class RotatingShieldPFConfig:
    ...
    strength_prior: StrengthPriorConfig | Mapping[str, object] = field(
        default_factory=lambda: ShiftedGammaStrengthPriorConfig(
            minimum_cps_1m=1.0,
            shape=2.0,
    ...
    planning_eig_samples: int | None = 50

    def __post_init__(self) -> None:
        """Validate and normalize estimator configuration values."""
        integer_fields = (
            ("num_particles", self.num_particles, 2),
            (
        ...
        if self.planning_eig_samples is not None:
            planning_samples = _strict_nonnegative_integer(
                self.planning_eig_samples,
                name="planning_eig_samples",
            )
            if planning_samples < 1:
                raise ValueError("planning_eig_samples must be at least 1.")
        ...
        if (
            not isinstance(self.gpu_device, str)
            or not self.gpu_device
            or self.gpu_device != self.gpu_device.strip()
            or self.gpu_device != self.gpu_device.lower()
        ):
            raise TypeError("gpu_device must be a canonical nonempty string.")
        ...
        if (
            _strict_config_number(
                ...
                name="joint_smc_rejuvenation_wall_time_limit_s",
            )
            <= 0.0
        ):
            raise ValueError(
                "joint_smc_rejuvenation_wall_time_limit_s must be positive."
            )
        ...
        if self.num_particles < 2:
            raise ValueError(
                "Production PF requires num_particles to be at least 2."
            )
        if self.gpu_dtype != "float64":
            raise ValueError("Pure PF production kernels require gpu_dtype='float64'.")
        self.strength_prior = resolve_strength_prior_config(self.strength_prior)
        ...
        if self.structural_cardinality_prior_probs is not None:
            ...
            if total_probability != 1.0:
                raise ValueError(
                    "structural_cardinality_prior_probs must sum exactly to 1."
                )
        ...
        if self.variable_cardinality:
            ...
            if self.num_particles < int(self.hard_max_sources) + 1:
                raise ValueError(
                    "num_particles must provide at least one initial particle "
                    "per cardinality from zero through hard_max_sources."
                )
        elif (
            initial_lower != initial_upper
            or initial_lower < 0
    ...
    def build_strength_prior(self) -> StrengthPrior:
        """Build the normalized strength prior from its discriminated config."""
        return self.strength_prior.build()
