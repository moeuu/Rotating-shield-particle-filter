...
This chapter presents the gamma-ray spectrum unfolding and automated radionuclide identification method used throughout this thesis. Given a one-dimensional energy spectrum measured by a mobile scintillation spectrometer, the objective is to decompose the spectrum into contributions from individual radionuclides and to estimate their relative activities. The target application is high-dose, cluttered environments in which a small ground robot must infer the three-dimensional distribution of γ-ray sources from non-directional, energy-resolved measurements.

Following the framework of Kemp et al. (2023), the measured spectrum is modelled as a linear superposition of responses from candidate radionuclides plus background, with Poisson counting statistics governing the observed bin counts. The detector response matrix encodes the expected contribution of each radionuclide to each energy bin, including full-energy peaks, energy resolution effects, and background components. On top of this model, the chapter adopts a practical, peak-based unfolding strategy similar to that of Anderson et al. (2019, 2022), in which individual photopeaks are detected, quantified, associated with library lines, and then mapped to radionuclide activities.

- Section 2.2 reviews basic concepts of ionising radiation, radioactive decay, and representative γ-emitting radionuclides relevant to nuclear power plant accidents and related scenarios.  
- Section 2.3 summarises typical radiation detectors, contrasts directional and non-directional instruments, and explains the choice of a compact, non-directional scintillation spectrometer mounted on a mobile robot.  
- Section 2.4 formulates the gamma-ray spectrum unfolding and radionuclide-identification problem as a linear inverse problem based on a detector response matrix and Poisson statistics.  
- Section 2.5 details the practical peak-based unfolding pipeline, including preprocessing, peak detection, baseline estimation, decomposition of overlapping peaks, dead-time correction, radionuclide library matching, and estimation of relative activities.  
- Section 2.6 summarises the chapter and links these components to the source-term estimation methods developed in subsequent chapters.
...
Representative types of radiation include particle radiation such as α rays, β rays, and neutron radiation, as well as electromagnetic radiation such as γ rays and X rays.

Each type of radiation has different energies and characteristics, and therefore different abilities to penetrate matter, as shown in Fig. 2.1. An α ray is a helium nucleus and can be stopped by a single sheet of paper. In air, it can travel only about 3 cm. Therefore, even if a source emitting α rays exists outside the human body, it can be shielded easily.

A β ray is a fast electron and exhibits a continuous energy spectrum, so β rays emitted from the same nuclide have different energies. A β ray with a maximum energy of about 1–2 MeV can penetrate approximately 2–4 mm of aluminum. For external exposure, the main concern is damage to the skin; β rays do not penetrate deeply into the body. However, when radioactive materials that emit α or β rays are taken into the body, internal exposure becomes an issue.

γ rays and X rays are electromagnetic waves with high penetrating power. Shielding them requires tens of centimetres of lead or several metres of concrete. When γ rays or X rays enter the body, some interact with tissues and lose their energy, while the rest pass through the body. X rays are widely used for radiographic diagnosis. γ rays can affect internal organs, and in external exposure the main concern is usually γ rays.
...
Because of their high penetrating power and strong adverse effects on the human body, this study focuses on γ rays as the target of measurement.

![Types of radiation and their penetrating power](Figures/chap2/Penetrating.png)

*Figure 2.1: Types of radiation and their penetrating power.*

### 2.2.2 Radioactive Materials

Among the above radiations, α rays, β rays, and γ rays are emitted in association with the decay of radioactive nuclides. When a nuclide undergoes α decay or β decay, it transforms into a different nuclide. In γ decay, the nuclide itself does not change, but its nuclear energy state changes.

Such decay occurs only once for each nucleus, following probabilistic laws, and each radioactive nuclide has a characteristic decay rate λ, which is the probability of decay per unit time. The rate of decrease of the number \(N\) of undecayed nuclei is proportional to \(N\) and can be expressed as

$$
-\frac{dN}{dt} = \lambda N,
$$

where λ is called the *decay constant*. If the initial number of nuclei is \(N_0\), the solution is
...
The time required for the number of radioactive nuclei to decrease to one half of its initial value is defined as the *half-life* \(T\). Substituting \(N = N_0/2\) into the above equation gives

$$
T = \frac{\ln 2}{\lambda}.
$$

As an example, through β decay, cesium-137 becomes the metastable nuclide barium-137m, which then emits a γ ray and transitions to stable barium-137.
...
The types of emitted radiation, representative γ-ray energies, and half-lives of these radionuclides are listed in Table 2.1.

Among them, cesium-137 is particularly problematic because it has a relatively long half-life of about 30 years and emits a strong 662 keV γ ray. Cesium-137 released into the environment remains for decades and often dominates the long-term dose. Cobalt-60 and europium-155 have shorter half-lives (on the order of 5 years) than cesium-137, but they can still persist in the environment for several years after an accident and contribute appreciably to the radiation field, especially inside reactor buildings and around activated components.

In realistic post-accident scenarios, these radionuclides may coexist, and appropriate countermeasures such as shielding, decontamination, and waste management depend on the specific isotopes present. Therefore, it is necessary to identify the radionuclides in the environment rather than relying only on total dose.

In this study, we focus on three representative γ-emitting radionuclides: cesium-137, cobalt-60, and europium-155. The spectrum unfolding and source-term estimation methods developed in the following chapters are designed to identify these isotopes and to estimate their spatial distributions and strengths.

**Table 2.1: Representative γ-ray energies and half-lives of selected radionuclides**

| Radionuclide   | Emitted radiation | Representative γ lines [keV]                   | Half-life     |
|---------------|-------------------|-----------------------------------------------|---------------|
| Cesium-137    | β⁻, γ             | 662                                           | 30.1 years    |
| Cesium-134    | β⁻, γ             | 605, 796                                      | 2.06 years    |
| Cobalt-60     | β⁻, γ             | 1173, 1332                                    | 5.27 years    |
| Europium-154  | β⁻, γ             | 723, 873, 996, 1275, 1494, 1596              | 8.6 years     |
| Europium-155  | β⁻, γ             | 86.5                                          | 4.76 years    |
| Radium-226    | α, γ              | 186                                           | 1600 years    |
| Iodine-131    | β⁻, γ             | 364                                           | 8.0 days      |
| Strontium-90  | β⁻                | —                                             | 29 years      |
| Plutonium-239 | α, γ              | 129, 375, 414                                 | 24,000 years  |
...
![Principle of a scintillation survey meter](Figures/chap2/Penetrating.png)

*Figure 2.2: Principle of a scintillation survey meter. Incident γ rays deposit energy in the scintillator, producing scintillation light that is converted into electrical pulses by a photomultiplier tube.*

#### Non-directional spectrometers

For source–term estimation and radionuclide identification, it is desirable to obtain not only the total count rate but also the energy spectrum of the detected γ rays. Non-directional spectrometers fulfil this requirement by combining a scintillation crystal (e.g., NaI(Tl) or CeBr₃) or a semiconductor detector (e.g., HPGe, CdZnTe) with multichannel pulse-height analysis electronics. From the measured spectrum, the characteristic photopeaks of individual radionuclides can be identified, enabling isotopic decomposition as described in this chapter.

In this thesis, the detector mounted on the mobile robot is a compact, energy-resolving, non-directional γ-ray spectrometer based on a scintillation crystal. It provides count-rate measurements over a wide dose-rate range comparable to that of survey meters, while simultaneously recording short-time energy spectra that are later processed by the spectrum-unfolding pipeline of Chapter 2.

![Example of a compact non-directional γ-ray spectrometer](Figures/chap2/Penetrating.png)

*Figure 2.3: Example of a compact non-directional γ-ray spectrometer based on a CeBr₃ scintillation crystal. The cylindrical detector head can be mounted on a mobile robot while providing energy-resolved spectra.*

### 2.3.2 Directional Detectors

Directional detectors are designed to measure not only the number of incident γ rays but also their approximate direction of arrival. By exploiting collimation, scattering kinematics, or coded apertures, these systems can directly form images of the radiation field. Examples include gamma cameras, Compton cameras, and various collimated or coded-aperture spectrometers.

In radiation source–distribution estimation, directional information greatly reduces the number of required measurement locations. However, most directional systems either require heavy shielding and collimators or have limited count-rate capability, which complicates their use on small mobile robots in high-dose environments.

#### Gamma camera

![Principle of a gamma camera](Figures/chap2/Penetrating.png)

*Figure 2.4: Principle of a gamma camera. A pixelated detector placed behind a lead pinhole collimator forms an inverted image of the radiation intensity on the detector plane.*

A pixelated scintillation or semiconductor detector is placed inside a lead shield with a pinhole or multi-pinhole collimator. Only γ rays that pass through the pinhole are detected, and an inverted map of radiation intensity is formed on the detector plane. Because the count in each pixel directly corresponds to the local intensity, subsequent image reconstruction is straightforward.
...
![Principle of a Compton camera](Figures/chap2/Penetrating.png)

*Figure 2.5: Principle of a Compton camera. (a) An incident γ ray undergoes Compton scattering in the scatterer and is absorbed in the absorber, defining a Compton cone. (b) Superposition of many cones allows the source position to be estimated.*

Unlike gamma cameras, Compton cameras do not rely on heavy collimators and can therefore be made relatively lightweight. A typical system consists of two detector layers: a **scatterer** and an **absorber**. An incident γ ray first undergoes Compton scattering in the scatterer; the scattered γ ray is then absorbed in the absorber. By measuring the energy deposits and interaction positions in both layers, the direction and energy of the incident γ ray can be reconstructed. Each detected event constrains the source to lie on a so-called Compton cone, and by superimposing many cones the source distribution can be estimated.

However, Compton cameras suffer from an upper limit on the usable count rate. In high-dose environments, multiple γ rays may enter the scatterer within a short time window, leading to overlapping interactions. When two or more Compton scattering events occur simultaneously, it becomes impossible to correctly associate the corresponding interactions in the scatterer and absorber, and the incident directions cannot be reconstructed reliably. For example, the Temporal Imaging Compton Camera V3 has an upper dose-rate limit of approximately 1 mSv/h, so it cannot be operated in the high-dose environments considered in this thesis.

### 2.3.3 Selection of Detector

In this thesis, the goal is to estimate the three-dimensional distribution of γ-ray sources in a high-dose environment using a small ground robot. The detector must therefore satisfy the following requirements:
...
For these reasons, this thesis employs a non-directional, energy-resolving scintillation spectrometer mounted on a mobile robot as the primary radiation sensor. A qualitative comparison of the detector types considered is summarised in Table 2.2. Because the chosen detector does not provide inherent directional information, the subsequent chapters develop methods that exploit attenuation by lightweight, actively rotated shields and environmental structures to recover pseudo-directional information and to perform three-dimensional source–term estimation.

**Table 2.2: Comparison of radiation detectors**
...
Consider a stationary or mobile gamma-ray detector that measures an energy spectrum in an environment where one or more radionuclides may be present. The detector output is discretised into \(K\) energy bins. Let \(\tilde{y}_k\) denote the observed number of counts in the \(k\)-th energy bin with central energy \(E_k\) and width \(\Delta E_k\) (\(k = 1,\dots,K\)). The measured spectrum is written as

$$
\tilde{\boldsymbol{y}} = (\tilde{y}_1, \tilde{y}_2, \dots, \tilde{y}_K)^\top.
$$

Assume that there exist \(M \geq 1\) candidate radionuclides. Let \(q_j\) denote a parameter proportional to the activity of the \(j\)-th radionuclide (for example, the activity at the detector position or the source strength after geometric attenuation), and define the vector

$$
\boldsymbol{q} = (q_1, q_2, \dots, q_M)^\top.
$$

Following Kemp et al. (2023), the expected number of counts in each energy bin is modelled as a linear combination of contributions from each radionuclide and from background:

$$
\boldsymbol{\mu}(\boldsymbol{q}) = \mathbf{R}\boldsymbol{q} + \boldsymbol{b},
$$

where \(\mathbf{R} \in \mathbb{R}^{K \times M}\) is the detector response matrix and

$$
\boldsymbol{b} = (b_1, b_2, \dots, b_K)^\top
$$

is the background spectrum (including environmental and intrinsic backgrounds).

The aim of spectrum unfolding is to estimate \(\boldsymbol{q}\) from \(\tilde{\boldsymbol{y}}\) and to determine which radionuclides are present. In the following, the structure of the response matrix \(\mathbf{R}\) and the statistical model of the spectrum are detailed.

### 2.4.1 Detector Response Matrix

For the \(j\)-th radionuclide, assume that the nuclear data library provides \(L_j\) discrete gamma lines. The \(\ell\)-th line has energy \(E_{j\ell}\) and emission probability (branching ratio) \(\beta_{j\ell}\) per decay. Let \(\epsilon(E)\) denote the full-energy peak detection efficiency at energy \(E\), and let \(\sigma(E)\) denote the energy resolution (standard deviation) of the detector, which is often approximated as
...
$$
R_{kj}
  = \sum_{\ell = 1}^{L_j} \beta_{j\ell}\,\epsilon(E_{j\ell})\,
    G\!\left(E_k; E_{j\ell}, \sigma(E_{j\ell})\right),
$$
...
In practice, \(R_{kj}\) may incorporate not only full-energy peaks but also Compton continua and scattering from the environment, either by Monte Carlo simulation or by experimental calibration.
...
Using \(\boldsymbol{\mu}(\boldsymbol{q}) = \mathbf{R}\boldsymbol{q} + \boldsymbol{b}\), the expected count in bin \(k\) is

$$
\mu_k(\boldsymbol{q}) = \sum_{j=1}^M R_{kj} q_j + b_k.
$$

### 2.4.2 Statistical Model

As gamma rays are emitted by stochastic radioactive decay processes, the number of counts in each bin is modelled as a Poisson random variable. Thus, for a given \(\boldsymbol{q}\),

$$
\tilde{y}_k \sim \mathrm{Poisson}\!\left(\mu_k(\boldsymbol{q})\right)
\quad (k = 1,\dots,K),
$$

where \(\mu_k(\boldsymbol{q})\) is the \(k\)-th element of \(\boldsymbol{\mu}(\boldsymbol{q})\).

Under this Poisson assumption, the likelihood of observing \(\tilde{y}_k\) counts in bin \(k\) given \(\boldsymbol{q}\) is

$$
p(\tilde{y}_k \mid \boldsymbol{q})
  = \frac{\mu_k(\boldsymbol{q})^{\tilde{y}_k}
          \exp\!\left(-\mu_k(\boldsymbol{q})\right)}
         {\tilde{y}_k!}.
$$
...
$$
p(\tilde{\boldsymbol{y}} \mid \boldsymbol{q})
  = \prod_{k=1}^K
      \frac{\mu_k(\boldsymbol{q})^{\tilde{y}_k}
            \exp\!\left(-\mu_k(\boldsymbol{q})\right)}
           {\tilde{y}_k!}.
...
In practice, direct optimisation of this likelihood over all bins and isotopes can be numerically challenging. Therefore, as in Kemp et al. (2024), a peak-based unfolding approach is adopted, in which the spectrum is first decomposed into individual peaks and then mapped to radionuclides.
...
7. Estimation of relative activities  

These steps closely follow the approach of Kemp et al. (2024) and Anderson et al. (2022).
...
$$
E(c) = a_0 + a_1 c + a_2 c^2,
$$

where \(a_0, a_1, a_2\) are calibration coefficients. These coefficients are estimated by least squares using one or more reference radionuclides with well-known peak energies. Let \((c_{r\ell}, E_{r\ell})\) be the channel location and known energy of the \(\ell\)-th reference peak. The calibration parameters are obtained by

$$
\hat{\boldsymbol{a}}
  = \operatorname*{arg\,min}_{\boldsymbol{a}}
    \sum_\ell
      \left(
        E_{r\ell} - (a_0 + a_1 c_{r\ell} + a_2 c_{r\ell}^2)
      \right)^2,
$$

where \(\boldsymbol{a} = (a_0, a_1, a_2)^\top\).
...
$$
\tilde{y}^{(\mathrm{sm})}_k
  = \sum_{m=-M}^{M} h_m\,\tilde{y}_{k-m},
$$

where the kernel coefficients \(h_m\) are given by
...
h_m
  = \frac{
      ...
      \sum_{r=-M}^{M}
        \exp\!\left(-\frac{r^2}{2\sigma_{\mathrm{sm}}^2}\right)
    },
$$

and \(\sigma_{\mathrm{sm}}\) controls the smoothing strength. Moderate smoothing improves peak detection robustness while preserving resolution.

### 2.5.2 Peak Detection

Peaks are detected on the smoothed spectrum \(\tilde{y}^{(\mathrm{sm})}_k\). One classical approach is the second-derivative method proposed by Mariscotti (1967). Define the discrete second difference

$$
D^2 \tilde{y}^{(\mathrm{sm})}_k
  = \tilde{y}^{(\mathrm{sm})}_{k+1}
    - 2\tilde{y}^{(\mathrm{sm})}_k
    + \tilde{y}^{(\mathrm{sm})}_{k-1}.
$$

For a locally linear background \(B(E) = \alpha + \beta E\), the second difference satisfies \(D^2 B \approx 0\); therefore, a negative value of \(D^2 \tilde{y}^{(\mathrm{sm})}_k\) indicates the presence of a peak. Using a noise estimate \(\sigma_{D^2}\), a channel \(k\) is declared a peak candidate if

$$
- D^2 \tilde{y}^{(\mathrm{sm})}_k > \lambda_{\mathrm{th}}\,\sigma_{D^2},
$$

where \(\lambda_{\mathrm{th}}\) is a user-defined signal-to-noise threshold.

In this thesis, peak detection is implemented using a Gaussian-matched filter. The filter correlates the spectrum with a normalised Gaussian template of width equal to the detector resolution, and local maxima in the filter response above a certain threshold are selected as peak candidates. This approach has been shown to perform robustly in the presence of Compton continua and moderate statistical noise.
...
The baseline is estimated using an asymmetric least-squares (ALS) smoothing method, which penalises deviations of the estimated baseline above the measured spectrum more strongly than deviations below it. Let \(y_k = \tilde{y}^{(\mathrm{sm})}_k\). The baseline \(\hat{b}_k\) is obtained by solving

$$
\hat{\boldsymbol{b}}
  = \operatorname*{arg\,min}_{\boldsymbol{b}}
    \left[
      \sum_{k=1}^K w_k (y_k - b_k)^2
      + \lambda \sum_{k=2}^{K-1} (\Delta^2 b_k)^2
    \right],
$$

where \(\Delta^2 b_k = b_{k+1} - 2b_k + b_{k-1}\) is the discrete second difference and \(\lambda\) is a smoothness parameter. The weights \(w_k\) are updated iteratively according to
...
For each detected peak, a local fitting window \(\mathcal{W}_p\) centred at energy \(E_p\) is defined, typically covering \(\pm 3\sigma_p\) where \(\sigma_p\) is estimated from the detector resolution. Within this window, the peak shape is modelled as a Gaussian:

$$
g_p(E; A_p, E_p, \sigma_p)
  = A_p \exp\!\left(-\frac{(E - E_p)^2}{2\sigma_p^2}\right),
$$

where \(A_p\) is the amplitude. The total model in the window is

$$
y_k \approx \hat{b}_k + g_p(E_k; A_p, E_p, \sigma_p).
$$

The parameters \((A_p, E_p)\) are obtained by least-squares fitting.

The net peak area \(N_p\) is then given by

$$
N_p = \sqrt{2\pi}\,A_p \sigma_p,
$$

or equivalently by discrete summation above the baseline,

$$
N_p = \sum_{k \in \mathcal{W}_p} (y_k - \hat{b}_k).
$$

Assuming Poisson statistics, the variance of \(N_p\) can be approximated as

$$
\sigma_{N_p}^2 \approx \sum_{k \in \mathcal{W}_p} y_k.
$$
...
When peaks from different radionuclides overlap owing to limited energy resolution, their contributions must be separated. Kemp et al. (2023) employ a spectral stripping approach based on known intensity ratios from the nuclear data library.

For each radionuclide \(j\), one **reference line** (for example the most intense or least overlapped peak) with energy \(E_{jr}\) is chosen. Let \(N_{jr}\) be its net area. For another line \(\ell\) of the same radionuclide, the expected net area is
...
Suppose that an observed peak at energy \(E_i\) is the sum of contributions from multiple radionuclides:

$$
N_i^{\mathrm{obs}} = \sum_j N_{ij}.
$$

If a subset of radionuclides has already been quantified via their reference lines, their contributions \(\hat{N}_{ij}\) can be predicted by the above relation and subtracted:

$$
N_i^{\mathrm{res}}
  = N_i^{\mathrm{obs}} - \sum_{j\in\mathcal{J}_{\mathrm{known}}} \hat{N}_{ij},
$$

where \(\mathcal{J}_{\mathrm{known}}\) is the set of radionuclides whose reference peaks are already assigned. The residual area \(N_i^{\mathrm{res}}\) is then used to estimate the remaining radionuclides. For example, Kemp et al. (2023) strip the contribution of the 609 keV line of the uranium/radium decay chain from the 662 keV region before quantifying Cs-137.

This stripping can be expressed as a linear system. Let \(\boldsymbol{N}^{\mathrm{obs}}\) be the vector of observed peak areas and \(\boldsymbol{\theta}\) the vector of reference-line areas for all radionuclides. Then

$$
\boldsymbol{N}^{\mathrm{obs}} \approx \mathbf{S}\boldsymbol{\theta},
$$

where \(\mathbf{S}\) is a matrix of intensity ratios. The least-squares estimate of \(\boldsymbol{\theta}\) is

$$
\hat{\boldsymbol{\theta}}
  = \operatorname*{arg\,min}_{\boldsymbol{\theta} \ge 0}
    \left\| \boldsymbol{N}^{\mathrm{obs}} - \mathbf{S}\boldsymbol{\theta} \right\|_2^2.
$$

The component \(\hat{\theta}_j\) corresponding to radionuclide \(j\) determines all its peak areas via the ratio relations above.

### 2.5.5 Dead-Time Correction

At high count rates, the detector and data acquisition system exhibit a dead time \(\tau_d\) during which additional pulses are not recorded. For a non-paralysable system, the relationship between the true count rate \(n\) and the measured count rate \(m\) is
...
$$
n_{\mathrm{tot}} = \frac{m_{\mathrm{tot}}}{1 - m_{\mathrm{tot}}\tau_d},
$$
...
f_{\mathrm{DT}}
  ...
  = \frac{1}{1 - m_{\mathrm{tot}}\tau_d}.
$$

Thus the dead-time-corrected bin counts are

$$
\tilde{y}^{(\mathrm{corr})}_k = f_{\mathrm{DT}}\tilde{y}_k,
$$
...
Having obtained a set of peak energies \(\{E_p\}\), net areas \(\{N_p\}\), and uncertainties \(\{\sigma_{N_p}\}\), each peak must be associated with candidate gamma lines from the radionuclide library.

#### Energy matching

Following Anderson et al. (2022), the difference between a measured peak energy \(E_\alpha\) and a library line energy \(E_\beta\) is denoted by

$$
d = |E_\alpha - E_\beta|.
$$

Let \(\sigma_\alpha\) and \(\sigma_\beta\) denote the standard uncertainties of the measured and library energies, respectively. To account for calibration drift and energy-dependent resolution, an empirical factor \(H(E)\) is introduced:
...
$$
\sigma_d^2
  = \zeta H(E)\left(\sigma_\alpha^2 + \sigma_\beta^2\right),
$$

where \(\zeta\) is a tuning parameter. Assuming \(d\) is normally distributed with mean zero and variance \(\sigma_d^2\), the normalised deviation is
...
For each radionuclide \(j\), let \(\mathcal{P}_j\) be the set of peaks whose energies are compatible with at least one gamma line of \(j\) according to the above criterion. Anderson et al. (2022) employ a Bayesian framework based on the method of Stinnett and Sullivan to compute the probability that radionuclide \(j\) is present.
...
using the peak areas in \(\mathcal{P}_j\) and their expected values under each hypothesis. The posterior probability of presence is then

$$
P(H_1 \mid \text{data})
  = \frac{
      L(\text{data} \mid H_1)\,P(H_1)
    }{
      L(\text{data} \mid H_0)\,P(H_0)
      + L(\text{data} \mid H_1)\,P(H_1)
    },
$$

where \(P(H_0)\) and \(P(H_1)\) are prior probabilities and \(L(\cdot)\) denotes the likelihood. A radionuclide is declared present if this posterior probability exceeds a threshold (e.g. 0.9).

### 2.5.7 Estimation of Relative Activities

Once a set of radionuclides has been identified as present, their relative activities are estimated using the net peak areas. For each peak \(p\) and radionuclide \(j\), the expected net area is

$$
\mu_{pj}
  = T \sum_\ell
      \beta_{j\ell}\,\epsilon(E_{j\ell})\,
      G\!\left(E_p; E_{j\ell}, \sigma(E_{j\ell})\right) q_j,
$$

where \(T\) is the live time. Restricting attention to the subset of peaks \(\mathcal{P}\) used for quantification, the vector of expected areas can be written in matrix form as

$$
\boldsymbol{\mu}_{\mathcal{P}} = \mathbf{M}\boldsymbol{q},
$$

where \(\boldsymbol{\mu}_{\mathcal{P}}\) stacks the expected areas of all peaks in \(\mathcal{P}\) and \(\mathbf{M}\) is the corresponding sensitivity matrix.

Let \(\boldsymbol{N}_{\mathcal{P}}\) be the vector of measured net areas and \(\mathbf{W}\) a diagonal weight matrix with elements

$$
W_{pp} = \frac{1}{\sigma_{N_p}^2}.
$$

The weighted least-squares estimate of \(\boldsymbol{q}\) is obtained by

$$
\hat{\boldsymbol{q}}
  = \operatorname*{arg\,min}_{\boldsymbol{q} \ge 0}
    \left\|
      \mathbf{W}^{1/2}
      \left(\boldsymbol{N}_{\mathcal{P}} - \mathbf{M}\boldsymbol{q}\right)
    \right\|_2^2.
$$

If \(\mathbf{M}^\top \mathbf{W}\mathbf{M}\) is non-singular, the solution has the closed form

$$
\hat{\boldsymbol{q}}
  = \left(\mathbf{M}^\top \mathbf{W}\mathbf{M}\right)^{-1}
    \mathbf{M}^\top \mathbf{W}\boldsymbol{N}_{\mathcal{P}}.
$$

In practice, non-negativity constraints and regularisation may be imposed to improve stability.
...
- Section 2.2 reviewed basic properties of ionising radiation, introduced representative radioactive materials, and highlighted key γ-emitting radionuclides relevant to post-accident environments.  
- Section 2.3 compared non-directional and directional radiation detectors and justified the use of a compact, non-directional scintillation spectrometer mounted on a mobile robot for high-dose measurements.  
- Section 2.4 modelled the measured spectrum using a detector response matrix and Poisson counting statistics, formulating spectrum unfolding as a linear inverse problem over radionuclide activities.  
- Section 2.5 detailed the practical peak-based unfolding procedure, including preprocessing, peak detection, baseline and overlapping-peak treatment, dead-time correction, radionuclide library matching, and estimation of relative activities.
...
- Section 3.3 explains how energy-resolved gamma-ray spectra are processed and converted into isotope-wise count sequences that serve as inputs to the PFs.  
- Section 3.4 formulates multi-isotope source-term estimation as Bayesian inference with parallel PFs, including the definition of precomputed geometric and shading kernels, state representation, prediction, log-domain weight update for Poisson observations, resampling, regularisation, and spurious-source rejection.  
- Section 3.5 presents the shield-rotation strategy, which selects informative shield orientations and next robot poses using information-gain and Fisher-information-based criteria together with short-time measurements.  
- Section 3.6 summarises the convergence criteria and the final outputs of the algorithm, which provide radiation maps for subsequent visualisation and decision making.  
...
## 3.3 Isotope-wise Count Sequence from Spectra

We assume that an energy–resolving detector (e.g., a scintillation detector) is used and that the gamma–ray spectrum is recorded at each measurement time. Let \(\tilde{z}_{k,c}\) denote the number of counts in channel \(c \in \{1,\dots,C\}\) at time step \(k\). After energy calibration, each channel corresponds to an energy \(E_c\).

A library of candidate isotopes \(\mathcal{H}\) is prepared. Each isotope \(h \in \mathcal{H}\) has a set of characteristic photopeaks with energies \(E_{h,p}\) and relative intensities (branching ratios) \(r_{h,p}\), where \(p = 1,\dots,P_h\). Following standard procedures, peak finding, baseline subtraction, and peak deconvolution are performed on the spectrum. For each peak \(p\), an energy window \(\mathcal{C}_{h,p}\) (a set of channels) is defined and the corresponding peak count is obtained as

$$
\tilde{y}_{k,h,p} = \sum_{c\in \mathcal{C}_{h,p}} \tilde{z}_{k,c}.
$$

Dead-time correction is applied to the recorded peak counts using the non-paralysable model described in Chapter 2, and the corrected values are again denoted by \(y_{k,h,p}\) for simplicity.

The contributions of multiple peaks belonging to the same isotope \(h\) are aggregated using the branching ratios as weights. The total count for isotope \(h\) at time \(k\) is defined as
...
$$
\boldsymbol{z}_k = \{z_{k,h}\}_{h\in\mathcal{H}}.
$$

These isotope–wise counts are used as observations for a set of parallel PFs, one PF per isotope, as in existing work on isotopic source–term estimation.
...
## 3.6 Convergence Criteria and Output
